    ...
    <article class="app-section-card-comfy">
      <div class="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div class="space-y-1">
          ...
          <h3 class="m-0 text-lg font-bold text-[rgb(var(--app-fg))]">Recent Single Service Transactions</h3>
          <p class="m-0 text-sm leading-6 text-[rgb(var(--app-fg))]/60">A quick view of the latest self pay single-service billings and their payment status.</p>
        </div>
      ...
      <div class="mt-4 overflow-hidden rounded-2xl border border-[rgb(var(--app-border))]">
        <DataTable :value="transactions" :loading="loading" size="small" dataKey="id" @row-click="onRowClick">
          <template #empty>
            <div class="px-4 py-10 text-center text-sm text-[rgb(var(--app-fg))]/60">No recent self pay single-service transactions yet.</div>
          </template>
          ...
          <Column header="Patient / Service" style="min-width: 260px">
            <template #body="{ data }">
              <div class="space-y-1">
                <div class="font-semibold">{{ data.patient_name || 'No patient linked' }}</div>
                <div class="text-xs text-[rgb(var(--app-fg))]/60">{{ data.service_name || 'Single Service' }}</div>
...
const loadDashboard = async (): Promise<void> => {
  ...
  try {
    ...
    error.value = getApiErrorMessage(err, {
      baseMessage: "Failed to load recent self pay single-service transactions",
