<template>
  <section class="app-section-card-comfy relative overflow-hidden">
    <div
      class="pointer-events-none absolute inset-0 bg-gradient-to-br from-sky-500/10 via-transparent to-emerald-500/10"
    />
    ...
    <div class="relative flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
      <div class="min-w-0 space-y-3">
        <div class="flex flex-wrap items-center gap-3">
          ...
          <div class="min-w-0">
            <h3 class="m-0 text-lg font-bold tracking-tight text-[rgb(var(--app-fg))] sm:text-xl">
              LGU Management Dashboard
            </h3>
        ...
        <p class="max-w-3xl text-sm leading-6 text-[rgb(var(--app-fg))]/65">
          Monitor LGU fund usage, review ledger transactions, manage program budgets,
          and open patient credit details from one focused workspace.
        </p>
      </div>

      <div class="flex w-full flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center xl:w-auto xl:justify-end">
        <IftaLabel class="w-full min-w-[240px] sm:w-64">
          <Select
            v-model="selectedProgramIdModel"
            :options="lguProgramOptions"
            optionLabel="label"
            optionValue="id"
            placeholder="All LGU programs"
        ...
        <Tag
          :value="canManageLguDashboard ? 'Manager Access' : 'Read-only'"
          :severity="canManageLguDashboard ? 'success' : 'secondary'"
          class="w-fit"
        />
