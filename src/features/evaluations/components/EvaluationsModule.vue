...
import {evaluationTanstackService} from "@/features/evaluations/queries/evaluation.tanstack.service";
...
const isOfferLoading = useIsLoading(EvaluationTanstackKey.EVALUATIONS)
const isExportLoading = useIsLoading(EvaluationTanstackKey.EVALUATIONS_EXPORT)
...
const selectedOffer = ref<Evaluation | undefined>()
...
} = usePaginationDebounce<Pageable<Evaluation> | undefined>()
...
const {
  ...
  error,
  refetch
} = evaluationTanstackService.getAll(requestParams)
useRefreshToken<Pageable<Evaluation> | undefined>(error, refetch)


const {
  mutate: toggleStatusMutation
} = evaluationTanstackService.toggleStatus()

const {
  mutate: saveMutation
} = evaluationTanstackService.save()

const {
  mutate: updateMutation
} = evaluationTanstackService.update()
...
const onExportToExcelThrottleFn = useThrottleFn(async (): Promise<void> => {
  ...
  const response: AxiosResponse<Blob> | undefined = await evaluationTanstackService.export(queryClient, request)
  if (!response) return
...
const menuButtons = (offer: Evaluation): MenuItem[] => {
  return [
    {
...
const resetQueries = async (): Promise<void> => {
  await queryClient.invalidateQueries({queryKey: [EvaluationTanstackKey.EVALUATIONS]})
}
</script>
