...
interface PatientService {
  ...
  save(body: PatientRequestBody): Promise<void | undefined>
...
export const patientService: PatientService = {
  ...
  async save(body: PatientRequestBody): Promise<void | undefined> {
    try {
      const {data: response} = await pamsAPI.post<void, AxiosResponse<void>, PatientRequestBody>(`${ResourceKey.PATIENTS}`, body)
      return response
    } catch (error: unknown) {
      errorHandler(error)
    }
