...
import {machineTanstackService} from "@/features/machines/queries/machine.tanstack.service";
...
const isOfferLoading = useIsLoading(MachineTanstackKey.MACHINES)
const isExportLoading = useIsLoading(MachineTanstackKey.MACHINES_EXPORT)
...
const selectedOffer = ref<Machine | undefined>()
...
} = usePaginationDebounce<Pageable<Machine> | undefined>()
...
const {
  ...
  error,
  refetch
} = machineTanstackService.getAll(requestParams)
useRefreshToken<Pageable<Machine> | undefined>(error, refetch)


const {
  mutate: toggleStatusMutation
} = machineTanstackService.toggleStatus()

const {
  mutate: saveMutation
} = machineTanstackService.save()

const {
  mutate: updateMutation
} = machineTanstackService.update()
...
const extractApiErrorMessage = (error: unknown, fallback: string): string => {
  return getApiErrorMessage(error, {
    ...
    permissionHint: "Machine access (Can Edit) in Role Access",
    invalidInputHint: "Machine values are invalid. Review name and price, then try again.",
...
const editFn = (payload: OfferEditPayload): void => {
  updateMutation(payload, {
    ...
    async onError(error: APIError) {
      errorToast(toast, extractApiErrorMessage(error, "Machine edit failed"))
      await resetQueries()
    },
  })
}
...
const saveFn = (request: OfferRequest): void => {
  saveMutation(request, {
    ...
    async onError(error: APIError) {
      errorToast(toast, extractApiErrorMessage(error, "Machine save failed"))
      await resetQueries()
    },
  })
}
...
const onExportToExcelThrottleFn = useThrottleFn(async (): Promise<void> => {
  ...
  const response: AxiosResponse<Blob> | undefined = await machineTanstackService.export(queryClient, request)
  if (!response) return
...
const menuButtons = (offer: Machine): MenuItem[] => {
  return [
    {
        ...
        confirm.require({
          ...
          accept: () => {
            ...
            toggleStatusMutation(selectedOffer.value.id, {
              ...
              async onError(error: APIError) {
                errorToast(toast, extractApiErrorMessage(error, "Toggle machine status failed"))
                await resetQueries()
              },
            })
          },
...
const resetQueries = async (): Promise<void> => {
  await queryClient.invalidateQueries({queryKey: [MachineTanstackKey.MACHINES]})
}
</script>
